...
CSV parser and the DOCX engine are implemented in-repo, which keeps the amount
of third-party code touching personal data small.
...
The DOCX engine does not use a general XML parser. It operates on `<w:t>` text
...
Every value in the test suite is synthetic. Templates and spreadsheets are
constructed in code rather than committed as binary fixtures, so the repository
contains no opaque files and no real personal data can be introduced through
one.
